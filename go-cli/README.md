...
boru-driven command-line client **and** interactive REPL for the Usercheck
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/usercheck-cli)
make build
...
./usercheck-cli load 1 domain            # {id:1} shorthand
./usercheck-cli load '{id:1}' domain       # explicit match map

# 5. Override the API base URL for a single call
USERCHECK_BASE=https://api.example.com ./usercheck-cli load 1 domain

# 6. No arguments -> interactive REPL
./usercheck-cli
usercheck> load 1 domain
usercheck> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/usercheck-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/usercheck-cli load 1 domain
   ```
...
```sh
./usercheck-cli load 1 domain          # scalar shorthand for {id:1}
./usercheck-cli load '{id:1}' domain     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `usercheck>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/usercheck-cli`. |
...
(`./usercheck-cli <expr>`) and interactively (the REPL), and expressions compose the
