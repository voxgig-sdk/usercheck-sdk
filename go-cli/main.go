// Command Usercheck-cli is a CLI + REPL for the Usercheck SDK,
...
const prompt = "usercheck"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "domain"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      USERCHECK_APIKEY (api key), USERCHECK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: USERCHECK_APIKEY carries the API key and
	// USERCHECK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("USERCHECK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("USERCHECK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUsercheckSDK(opts)
