...
MCP server exposing the Usercheck SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `usercheck_list` | `entity`, optional `query` map | First page of records as JSON |
| `usercheck_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `usercheck_list` and
`usercheck_load` tools become available in new sessions.
