...
require_relative 'Usercheck_types'
...
class UsercheckSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UsercheckHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UsercheckHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UsercheckFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UsercheckUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UsercheckHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UsercheckHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UsercheckHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UsercheckHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UsercheckError.new(
        "graphql_error", "UsercheckSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
