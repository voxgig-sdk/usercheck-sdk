...
module UsercheckUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UsercheckResponse.new({})
    ...
    if fetch_err
      ...
      response = UsercheckResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UsercheckResponse.new(fetched)
    else
