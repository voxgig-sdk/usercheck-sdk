...
class DomainEntity
  ...
  def data_set(args)
    if args
      @_data = UsercheckHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = UsercheckHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Domain, Hash] the loaded Domain; raises UsercheckError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UsercheckHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
