...
class DomainEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set USERCHECK_TEST_DOMAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function domain_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("USERCHECK_TEST_DOMAIN_ENTID");
    ...
    if ($env["USERCHECK_TEST_LIVE"] === "TRUE") {
        ...
        $client = new UsercheckSDK(Helpers::to_map($merged_opts));
    }
