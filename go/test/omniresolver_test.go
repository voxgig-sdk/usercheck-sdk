...
import (
	...
	omni "github.com/voxgig-sdk/usercheck-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UsercheckError); is {
				out := map[string]any{
					"name":    "UsercheckError",
...
func omniCtx(arg any, client *sdk.UsercheckSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UsercheckSDK); is {
			client = live
