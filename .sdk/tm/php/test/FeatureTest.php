...
class FtCtrl extends UsercheckControl
{
...
class FtClient
{
    ...
    private UsercheckUtility $utility;

    public function __construct(UsercheckUtility $utility, array $options)
    {
    ...
    public function get_utility(): UsercheckUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UsercheckContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UsercheckContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new UsercheckUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (UsercheckContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UsercheckConfig::make_feature($name);
    ...
    private static function buildUrl(UsercheckSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UsercheckError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UsercheckResponse($fetched);
        }
    ...
    private function fail(UsercheckContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UsercheckContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
