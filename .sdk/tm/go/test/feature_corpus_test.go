...
//
// NewUsercheckSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.UsercheckSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewUsercheckSDK(opts)
}
...
func fcCandidates(client *sdk.UsercheckSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.UsercheckSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.UsercheckSDK, name string) any {
