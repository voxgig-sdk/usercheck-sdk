...
//     (\`new UsercheckSDK(...)\` / \`UsercheckSDK.test(...)\` become
//     \`UsercheckSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"domain":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UsercheckSDK(...)\` and any
// \`UsercheckSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UsercheckSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UsercheckSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
